...
# ---------------------------------------------------------------------------
# PURCHASE ORDER STATEMENT - currencies shown separately, never summed together
# ---------------------------------------------------------------------------
def _po_totals_by_currency(pos):
    totals = {}
    for po in pos:
        totals[po.currency] = totals.get(po.currency, 0) + po.grand_total
    return totals


def build_po_excel_response(queryset, title_period='All Purchase Orders'):
    from openpyxl import Workbook
    from openpyxl.styles import Font, Alignment, PatternFill, Border, Side
    from openpyxl.utils import get_column_letter

    pos = list(queryset.prefetch_related('line_items').order_by('po_no'))
    totals = _po_totals_by_currency(pos)
    count = len(pos)
    ...
    ws.merge_cells('A2:F2')
    ws['A2'] = f'Purchase Order Statement — {title_period}'
    ws['A2'].font = Font(bold=True, size=11)
    ws['A2'].alignment = center

    ws.merge_cells('A4:B4')
    ws['A4'] = f'Total POs: {count}'
    ws['A4'].font = bold

    totals_text = '   '.join(f'{cur}: {amt:,.2f}' for cur, amt in totals.items()) or 'No POs'
    ws.merge_cells('C4:F4')
    ws['C4'] = f'Totals — {totals_text}'
    ws['C4'].font = Font(bold=True, size=11, color='1D4ED8')
    ws['C4'].alignment = Alignment(horizontal='right')

    headers = ['S.No', 'PO Ref No', 'Date', 'Supplier', 'Currency', 'Grand Total']
    header_row = 6
    for col, h in enumerate(headers, start=1):
        cell = ws.cell(row=header_row, column=col, value=h)
        cell.font = header_font
    ...
    for i, po in enumerate(pos, start=1):
        ws.cell(row=row, column=1, value=i).border = border
        ws.cell(row=row, column=1).alignment = center
        ws.cell(row=row, column=2, value=po.po_no).border = border
        ws.cell(row=row, column=3, value=po.po_date.strftime('%d-%m-%Y')).border = border
        ws.cell(row=row, column=4, value=po.supplier.name).border = border
        ws.cell(row=row, column=5, value=po.currency).border = border
        gt_cell = ws.cell(row=row, column=6, value=float(po.grand_total))
        gt_cell.number_format = '#,##0.00'
        gt_cell.border = border
        row += 1

    widths = [6, 20, 12, 26, 10, 16]
    for i, w in enumerate(widths, start=1):
        ws.column_dimensions[get_column_letter(i)].width = w
    ...
    wb.save(buffer)
    buffer.seek(0)
    filename = f'PO_Statement_{title_period.replace(" ", "_")}.xlsx'
    response = HttpResponse(
        buffer,
        content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    )
...
def build_po_pdf_response(queryset, title_period='All Purchase Orders'):
    from reportlab.lib.pagesizes import A4, landscape
    from reportlab.lib.units import mm
    from reportlab.lib import colors
    from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, Spacer
    ...
    elements = []
    elements.append(Paragraph('VIRTUAL INSTRUMENTATION &amp; SOFTWARE APPLICATIONS PVT LTD.', s_company))
    elements.append(Paragraph(f'Purchase Order Statement &mdash; {title_period}', s_title))
    elements.append(Spacer(1, 3 * mm))

    totals_text = '&nbsp;&nbsp;&nbsp;'.join(f'{cur}: {amt:,.2f}' for cur, amt in totals.items()) or 'No POs'
    top_row = [[
        Paragraph(f'Total POs: <b>{count}</b>', s_meta),
        Paragraph(totals_text, s_total),
    ]]
    usable_w = page_w - 24 * mm
    top_table = Table(top_row, colWidths=[usable_w * 0.5] * 2)
    top_table.setStyle(TableStyle([
    ...
    for i, po in enumerate(pos, start=1):
        data.append([
            ...
            Paragraph(po.po_no, s_cell),
            Paragraph(po.po_date.strftime('%d-%m-%Y'), s_cell),
            Paragraph(po.supplier.name, s_cell),
            Paragraph(po.currency, s_cell),
            Paragraph(f'{po.grand_total:,.2f}', s_cell_r),
        ])

    col_widths = [usable_w * w for w in [0.06, 0.20, 0.12, 0.34, 0.12, 0.16]]
    table = Table(data, colWidths=col_widths, repeatRows=1)
    table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#1D4ED8')),
    ...
    doc.build(elements)
    buffer.seek(0)
    filename = f'PO_Statement_{title_period.replace(" ", "_")}.pdf'
    response = HttpResponse(buffer, content_type='application/pdf')
    response['Content-Disposition'] = f'inline; filename="{filename}"'
    return response